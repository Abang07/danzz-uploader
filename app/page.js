...
export default function Home() {
  ...
  async function uploadFile(file) {
    ...
    try {
      ...
      const res = await fetch(`https://api.telegram.org/bot${process.env.NEXT_PUBLIC_TELEGRAM_BOT_TOKEN}/sendDocument`, {
        method: 'POST',
        body: telegramForm,
      });
      ...
      if (!data.ok) {
        setError(data.description || 'Upload gagal');
        return;
      }
      ...
      if (file.size <= 20 * 1024 * 1024) {
        const fileRes = await fetch(`https://api.telegram.org/bot${process.env.NEXT_PUBLIC_TELEGRAM_BOT_TOKEN}/getFile?file_id=${fileId}`);
        const fileData = await fileRes.json();
        url = `https://api.telegram.org/file/bot${process.env.NEXT_PUBLIC_TELEGRAM_BOT_TOKEN}/${fileData.result.file_path}`;
      }

      if (url) setResults(prev => [{ url, name: file.name, size: file.size, id: Date.now() }, ...prev]);
      else setError('File terkirim ke Telegram tapi URL tidak tersedia (>20MB)');
